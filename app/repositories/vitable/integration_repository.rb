module Vitable
  class IntegrationRepository < ApplicationRepository
    ...
    def payroll_deduction_payload(event)
      payload = RemotePayrollDeductionDto.normalized_payload(event.payload)

      payload.merge(
        "id" => payload.fetch("id", nil).presence ||
          payload.fetch("deduction_id", nil).presence ||
          event.resource_id
      )
    end
    ...
    def snapshot_counts(snapshot)
...

      {
        ...
        "remote_employee_dependent_count" => employee_reconciliation.fetch("dependent_processed_count", 0),
        "reconciled_employee_dependent_count" => employee_reconciliation.fetch("dependent_matched_count", 0),
        "created_employee_dependent_count" => employee_reconciliation.fetch("dependent_created_count", 0),
        "updated_employee_dependent_count" => employee_reconciliation.fetch("dependent_updated_count", 0),
        "employee_dependent_missing_required_count" => employee_reconciliation.fetch("dependent_missing_required_count", 0),
