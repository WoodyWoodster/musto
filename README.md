...
The Vitable employer provisioning workspace builds the documented `POST /v1/employers` payload and, once a remote employer ID exists, switches to the documented settings update path for pay frequency. Eligibility classification and waiting-period data are maintained as a local readiness profile, submitted through the documented eligibility policy path, and reconciled from Vitable webhook events. It uses DTO-backed packet review, repository-generated holdbacks, and `needs_credentials` sync runs so the create path can be proofed before an API key exists.

The Vitable census sync workspace builds a local review manifest for `POST /v1/employers/:id/census-sync`, separates ready employee rows from holdbacks, and records submit attempts as `SyncRun` rows. Accepted census submissions are stored as DTO-backed read state, and remote roster refresh reconciles Vitable employee IDs, member IDs, status, and deduction snapshots back into local employee metadata. Without `VITABLE_CONNECT_API_KEY`, submits are accepted by the app as `needs_credentials` runs so the full workflow can be proofed before live credentials exist.
...
Without `VITABLE_CONNECT_API_KEY`, the event is accepted and marked `needs_credentials`. If `VITABLE_WEBHOOK_SECRET` is configured, send `X-Vitable-Timestamp` and a timestamp-bound HMAC-SHA512 `X-Vitable-Signature` header with the webhook request.
