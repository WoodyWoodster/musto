...
class OperationsWorkflowsTest < ActionDispatch::IntegrationTest
  ...
  test "integration connection workspace exposes credential and coverage DTOs" do
    ...
    assert_equal Vitable::Configuration::DEMO_API_BASE_URL, detail.api_base_url
    assert_equal Vitable::Configuration::DEFAULT_ENVIRONMENT, detail.sdk_environment
    api_target_check = detail.health_checks.find { |check| check.label == "API target" }
    assert_equal "ready", api_target_check.status
    assert_equal "Requests target #{Vitable::Configuration::DEMO_API_BASE_URL}", api_target_check.detail
    ...
    assert_select "p", Vitable::Configuration::DEMO_API_BASE_URL
  ...
  test "successful Vitable API snapshot refresh stores remote read counts" do
    ...
    external_org_event = WebhookEvent.find_by!(event_id: "wevt_remote_external_org")
    assert_equal @organization.external_id, external_org_event.organization_external_id
  ...
  test "Vitable API snapshot refresh mirrors mapped employer detail responses" do
    ...
    assert result.success?, result.errors.to_sentence
    snapshot = @connection.reload.metadata.fetch("api_snapshot")
    detail = snapshot.fetch("employer_details").sole

    assert_equal 1, snapshot.dig("counts", "remote_employer_count")
    assert_equal 1, snapshot.dig("counts", "retrieved_remote_employer_count")
    assert_equal 0, snapshot.dig("counts", "errored_remote_employer_detail_count")
    ...
    assert_equal 1, dto.retrieved_remote_employer_count
    assert_equal 0, dto.errored_remote_employer_detail_count
  ensure
    ENV[@connection.api_key_reference] = previous_key
  end
